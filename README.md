# 鈴子ちゃん公式サイト

GitHub Pages で公開している、オリジナルキャラクター「鈴子ちゃん」の公式ウェブサイトのリポジトリです。

## 公式サイト URL

https://abcderp2.github.io/suzukochan.officialsite/

---

## 鈴子ちゃんについて

鈴子ちゃんは、若手看護師という設定を持つオリジナルキャラクターです。
白を基調とした看護師風の服装と、紺色の髪が特徴。年相応の無邪気な性格と可愛い笑顔が魅力的なキャラクターです。

### 基本情報

- フルネーム：鈴木 鈴子（すずき すずこ）
- 呼び名：鈴子ちゃん
- 年齢：21歳（永遠に）
- 生年月日：2004年7月15日（設定）
- 職業：架空の病院 White Wing（白い翼）病院に勤める若手看護師
- イメージカラー：白

詳しいプロフィールや誕生の経緯は、公式サイトの「プロフィール」「鈴子ちゃんについて」セクションをご覧ください。

---

## サイトのコンテンツ

### ギャラリー

鈴子ちゃんの公式ビジュアル（バージョン1、バージョン2）を公開しています。

### ガイドライン

二次創作やファンアート制作時のルールを詳しく解説しています。
商用利用やR-18表現の禁止など、キャラクターを大切に守るためのガイドです。

### 更新情報

サイトの更新履歴や、キャラクター関連の最新情報を掲載しています。

---

## 各種SNS・関連リンク

キャラクターや作者に関する最新情報はこちらでも発信しています。

- YouTube: https://www.youtube.com/@abc-l2g6k/shorts
- note（作者について）: https://note.com/stocktrading0_ai
- X（作者 鈴木）: https://x.com/stocktrading0
- X（鈴子ちゃん）: https://x.com/suzuko_ai_/

---

## 技術スタック

このサイトはシンプルで軽量な構成を目指しています。

- HTML5：セマンティックなマークアップ
- CSS3：レスポンシブデザイン対応
- GitHub Pages：ホスティング
- ビルドツール：なし（静的サイト）

## サイト保守

保守に必要な事実、AIへの依頼文、スマートフォンとタブレットでの手順、検査、復旧方法は、[MAINTENANCE.md](MAINTENANCE.md)に統合しています。

一般的な無料プランのAIとエントリークラスのスマートフォンまたはタブレットだけでも進められる手順です。保守を依頼するAIには、最初に`MAINTENANCE.md`と今回の依頼内容を渡してください。

### 画像のプライバシー保護\n\n画像をサイトへ追加する前に、[画像の個人情報を削除](image-privacy.html)を開いてください。画像は端末内だけで処理され、Exifなどの画像メタデータを削除した画像を、元のファイル名のままダウンロードできます。元の画像ではなく、ダウンロードした画像をGitHubへアップロードしてください。複数枚ある場合は1枚ずつ処理します。\n\nJPEG、PNG、WebPに対応しています。サイトの自動検査でもメタデータを確認し、未処理の画像を見つけた場合は公開前に止めます。公開ページの更新情報にはこの機能追加を記載しません。\n\n### ローカル確認

公開前にローカルで表示を確認する場合は、次のコマンドを実行します。

```bash
git clone https://github.com/abcderp2/suzukochan.officialsite.git
cd suzukochan.officialsite
python3 -m http.server 8000
```

起動後、ブラウザで`http://localhost:8000/`を開きます。終了する場合は、ターミナルで`Ctrl+C`を押します。
